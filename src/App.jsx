...
export default function OrcinusLanding() {
      ...
      window.google.accounts.id.initialize({
        ...
        callback: (response) => {
          ...
          if (ADMIN_EMAILS.includes(payload.email)) {
            ...
            alert(t[lang]?.admin?.notAdmin || '관리자 계정이 아닙니다.');
          }
        },
      });
    };
  ...
  const t = {
    ko: {
      nav: { features: '특징', services: '서비스', contact: '문의', demo: '데모 신청', disclosure: '공시', about: '회사소개' },
      langLabel: '한국어',
      logoToggle: { toOrca: '돌고래로 전환', toLogo: '로고로 전환' },
      hero: {
      ...
      stats: {
        ...
        integrated: '통합', integratedLabel: '올인원 플랫폼'
      },
      features: {
        badge: 'WHY ORCA',
        title: '왜',
        ...
        items: [
          { title: "실시간 손익 & 기준가", desc: "운용역별/펀드별 실시간 기준가 업데이트" },
          { title: "국내/해외 실시간 체결", desc: "해외 자산도 당일 실시간 반영, 장중 진입/청산 가능" },
          { title: "실시간 리스크 관리", desc: "실시간 반영으로 즉각적인 리스크 체크 가능" },
          { title: "Active ETF 운용", desc: "실시간 PDF 전송, 설정환매, 대여리콜 관리" },
          { title: "멀티매니저 운용", desc: "매니저별 손익/잔고 관리, 반대방향 주문 자동상계 후 동시진행" },
        ]
      },
      clients: {
      ...
      modules: {
        ...
        inquire: '상세 문의하기',
        items: [
          { title: "포트폴리오 관리", desc: "실시간 기준가 산출 및 펀드 성과 분석", details: ["실시간 기준가 관리", "멀티 매니저 지원", "펀드 마감/성과 분석", "포트폴리오 리밸런싱", "벤치마크 비교 분석"] },
          { title: "주문 관리", desc: "주문 생성부터 체결까지 통합 관리", details: ["주문 상계 처리", "리스크 관리", "주문 제약 위반 방지", "배분 규칙 자동화", "컴플라이언스 체크"] },
          { title: "주문 집행", desc: "알고리즘 트레이딩 및 최적 체결", details: ["TWAP/VWAP 알고리즘", "Smart Order Routing", "거래 비용 최적화", "실시간 체결 모니터링", "TCA 분석"] },
          { title: "대차 관리", desc: "차입/대여 통합 관리 시스템", details: ["차입/대여/대용 관리", "대차 허브 연동", "대차 프로세스 자동화", "담보 관리", "리콜 처리"] },
          { title: "ETF 관리", desc: "Active ETF 운용 특화 시스템", details: ["Active ETF 운용", "실시간 PDF 전송", "설정환매/리콜 관리", "AP 연동", "NAV 실시간 계산"] },
          { title: "내부 잔고 관리", desc: "실시간 잔고 및 공매도 체크", details: ["실시간 잔고관리", "공매도 체크", "거래 내역 기록", "T+2 결제 관리", "잔고 불일치 알림"] },
          { title: "시장 데이터", desc: "국내/해외 실시간 시세 제공", details: ["국내/해외 실시간 시세", "기준가 실시간 반영", "해외 자산 당일 반영", "환율 정보", "지수 데이터"] },
        ]
      },
      flow: {
        badge: 'ARCHITECTURE',
      ...
      deploy: {
        ...
        subtitleLong: '고객사의 보안 정책과 인프라 환경에 맞춰 유연하게 선택',
        options: [
          { title: "SaaS", desc: "클라우드 기반 서비스", features: ["별도 인프라 구축 불필요", "빠른 도입 가능", "자동 업데이트 지원"] },
          { title: "고객사 클라우드", desc: "AWS · Azure · GCP", features: ["고객사 클라우드 환경 설치", "유연한 리소스 확장", "클라우드 보안 정책 적용"] },
          { title: "온프레미스", desc: "자체 데이터센터 설치", features: ["완전한 데이터 통제", "내부망 운영 가능", "자체 보안 정책 적용"] },
        ],
        footnote: '필요한 모듈만 선택적으로 도입하여 서비스 도입 부담을 줄이고, 기존 시스템을 순차적으로 대체할 수 있습니다.'
      },
      about: {
        badge: 'ABOUT ORCINUS',
        title: '에 대하여',
        subtitle: '자산운용의 미래를 함께 만들어갑니다',
        sectionTitle: '자산운용의 정점',
        sectionDesc1: 'Orcinus는 국내 최고 수준의 자산운용 인프라를 제공합니다.',
        sectionDesc2: '복잡한 운용 환경에서도 안정적이고 효율적인 시스템을 경험하세요.',
        stats: [
        ...
        coreValues: '핵심 가치',
        coreValueItems: [
          { title: "위험 관리", desc: "실시간 기준가 기반 시장 위험 관리\n공매도 법적 처벌 방지" },
          { title: "비용 절감", desc: "Buy-side 주문 집행으로 거래 비용 최적화\n대차 비용 자동 최소화" },
          { title: "업무 자동화", desc: "주문/대차/백오피스 자동화\n운용 지원 인력 생산성 극대화" },
        ],
        aboutTitle: '자산운용의 미래를\n함께 만들어갑니다',
        aboutDesc1: 'Orcinus는 자산운용사가 더 나은 투자 결정을 내릴 수 있도록',
        aboutDesc2: '기술로 혁신합니다',
      ...
      footer: {
        ...
        tagline: 'Orca - 실시간 통합 자산운용 플랫폼',
        system: '시스템',
        systemLinks: ['PMS/OMS/EMS', '대차/ETF관리'],
        companyTitle: '회사',
        companyLinks: ['소개', '채용'],
        contactTitle: '문의'
      },
      demo: {
        title: '데모 신청',
        desc: '담당자가 확인 후 연락드리겠습니다.',
        nameLabel: '이름',
        namePlaceholder: '홍길동',
        companyLabel: '회사명',
        companyPlaceholder: '(주) 오르시너스',
        phoneLabel: '연락처',
        emailLabel: '이메일',
        messageLabel: '문의 내용',
        messagePlaceholder: '도입 환경, 운용 규모, 궁금한 점 등을 자유롭게 작성해주세요.',
        errorMsg: '전송 중 오류가 발생했습니다. 다시 시도해주세요.',
        sending: '전송 중...',
        submit: '데모 신청하기',
        required: '* 표시 항목은 필수입력입니다.',
        successTitle: '신청이 완료되었습니다!',
        successDesc: '담당자가 빠른 시일 내에 연락드리겠습니다.',
        successBtn: '확인'
      },
      admin: {
        loginTitle: '관리자 로그인',
        loginDesc: '허용된 구글 계정으로만 접근 가능합니다',
        notAdmin: '관리자 계정이 아닙니다.',
        logout: '로그아웃',
        login: '관리자 로그인'
      },
      disclosure: {
        title: '공시 게시판',
        desc: 'Orcinus의 공지사항과 공시 정보를 확인하세요',
        notice: '공지사항',
        general: '일반공시',
        demoTab: '데모신청',
        loading: '불러오는 중...',
        noDemo: '아직 데모 신청이 없습니다.',
        adminOnly: '관리자 전용',
        total: '총',
        unit: '건',
        nameCol: '이름',
    ...
    en: {
      nav: { features: 'Features', services: 'Services', contact: 'Contact', demo: 'Request Demo', disclosure: 'Disclosure', about: 'About' },
      langLabel: '한국어',
      logoToggle: { toOrca: 'Switch to Orca', toLogo: 'Switch to Logo' },
      hero: {
      ...
      stats: {
        ...
        integrated: 'Integrated', integratedLabel: 'All-in-One Platform'
      },
      features: {
        badge: 'WHY ORCA',
        title: 'Why',
        ...
        items: [
          { title: "Real-time P&L & NAV", desc: "Real-time NAV updates per fund manager and fund" },
          { title: "Real-time Domestic/Overseas Execution", desc: "Same-day real-time reflection of overseas assets, intraday entry/exit available" },
          { title: "Real-time Risk Management", desc: "Instant risk checks with real-time data reflection" },
          { title: "Active ETF Operations", desc: "Real-time PDF transmission, creation/redemption, lending recall management" },
          { title: "Multi-Manager Operations", desc: "P&L/position management per manager, automatic netting of opposite orders" },
        ]
      },
      clients: {
        badge: 'ORCINUS SERVICE',
        ...
        items: [
          ...
          { title: "Discretionary Investment Manager" },
          { title: "SMA" },
          { title: "Securities Firm" },
        ]
      },
      modules: {
        badge: 'ORCA MODULES',
        ...
        inquire: 'Contact for Details',
        items: [
          { title: "Portfolio Management", desc: "Real-time NAV calculation and fund performance analysis", details: ["Real-time NAV Management", "Multi-Manager Support", "Fund Closing/Performance Analysis", "Portfolio Rebalancing", "Benchmark Comparison Analysis"] },
          { title: "Order Management", desc: "Integrated management from order creation to execution", details: ["Order Netting", "Risk Management", "Order Constraint Violation Prevention", "Allocation Rule Automation", "Compliance Check"] },
          { title: "Order Execution", desc: "Algorithmic trading and optimal execution", details: ["TWAP/VWAP Algorithms", "Smart Order Routing", "Transaction Cost Optimization", "Real-time Execution Monitoring", "TCA Analysis"] },
          { title: "Securities Lending & Borrowing", desc: "Integrated borrowing/lending management system", details: ["Borrowing/Lending/Collateral Management", "SLB Hub Integration", "SLB Process Automation", "Collateral Management", "Recall Processing"] },
          { title: "ETF Management", desc: "Specialized system for Active ETF operations", details: ["Active ETF Operations", "Real-time PDF Transmission", "Creation/Redemption & Recall Management", "AP Integration", "Real-time NAV Calculation"] },
          { title: "Internal Position Management", desc: "Real-time positions and short selling check", details: ["Real-time Position Management", "Short Selling Check", "Transaction History Recording", "T+2 Settlement Management", "Position Discrepancy Alerts"] },
          { title: "Market Data", desc: "Real-time domestic/overseas market data", details: ["Domestic/Overseas Real-time Quotes", "Real-time NAV Reflection", "Same-day Overseas Asset Reflection", "FX Rate Information", "Index Data"] },
        ]
      },
      flow: {
        badge: 'ARCHITECTURE',
      ...
      deploy: {
        ...
        subtitleLong: 'Choose flexibly based on your security policies and infrastructure',
        options: [
          { title: "SaaS", desc: "Cloud-based Service", features: ["No separate infrastructure required", "Rapid deployment", "Automatic updates"] },
          { title: "Client Cloud", desc: "AWS · Azure · GCP", features: ["Deployed in client cloud environment", "Flexible resource scaling", "Cloud security policy applied"] },
          { title: "On-Premise", desc: "Self-hosted Data Center", features: ["Complete data control", "Internal network operation", "Custom security policies"] },
        ],
        footnote: 'Selectively adopt only the modules you need to reduce deployment burden and gradually replace existing systems.'
      },
      about: {
        badge: 'ABOUT ORCINUS',
        title: 'About Orcinus',
        subtitle: 'Building the future of asset management together',
        sectionTitle: 'The Pinnacle of Asset Management',
        sectionDesc1: 'Orcinus provides best-in-class asset management infrastructure.',
        sectionDesc2: 'Experience a stable and efficient system even in complex operating environments.',
        stats: [
          { value: "Real-time", label: "NAV Updates", desc: "Instant reflection per fund manager/fund" },
          { value: "Multi", label: "Manager Operations", desc: "Reduced trading costs through manager-level netting" },
          { value: "100%", label: "Short Selling Check", desc: "Real-time position management with borrowing/lending/collateral" },
          { value: "Web", label: "Access Anywhere", desc: "No mobile/PC installation required" },
        ],
        coreValues: 'Core Values',
        coreValueItems: [
          { title: "Risk Management", desc: "Real-time NAV-based market risk management\nShort selling legal compliance" },
          { title: "Cost Reduction", desc: "Transaction cost optimization via buy-side execution\nAutomatic SLB cost minimization" },
          { title: "Workflow Automation", desc: "Order/SLB/back-office automation\nMaximized operations support productivity" },
        ],
        aboutTitle: 'Building the Future of\nAsset Management Together',
        aboutDesc1: 'Orcinus innovates with technology so that asset managers',
        aboutDesc2: 'can make better investment decisions',
        missionDesc: 'We simplify complex asset management workflows and support real-time data-driven decision making to maximize investment performance.',
        ...
        coreValuesList: [
          { desc: "Fast execution and response" },
          { desc: "Reliable systems" },
          { desc: "Continuous innovation" },
          { desc: "Growing with our clients" },
        ],
        timeline: [
          { year: "2024", title: "Orca Official Launch", desc: "Integrated trading system launched for asset managers" },
          { year: "2023", title: "Orcinus Founded", desc: "Financial IT professionals came together to establish the company" },
        ],
        teamTitle: 'Our Team',
        teamDesc: 'Experts in finance and technology working together',
        teamMembers: [
          { expertise: "20 Years in Asset Management" },
          { expertise: "15 Years in Financial IT" },
          { expertise: "Trading Systems Specialist" },
        ]
      },
      ...
      footer: {
        ...
        copy: '© 2025 Orcinus. All rights reserved.',
        tagline: 'Orca - Real-time Integrated Asset Management Platform',
        system: 'System',
        systemLinks: ['PMS/OMS/EMS', 'SLB/ETF Management'],
        companyTitle: 'Company',
        companyLinks: ['About', 'Careers'],
        contactTitle: 'Contact'
      },
      demo: {
        title: 'Request Demo',
        desc: 'Our team will review and contact you.',
        nameLabel: 'Name',
        namePlaceholder: 'John Doe',
        companyLabel: 'Company',
        companyPlaceholder: 'Orcinus Inc.',
        phoneLabel: 'Phone',
        emailLabel: 'Email',
        messageLabel: 'Message',
        messagePlaceholder: 'Please describe your deployment environment, AUM, and any questions.',
      ...
      admin: {
        ...
        loginDesc: 'Access restricted to authorized Google accounts only',
        notAdmin: 'This is not an admin account.',
        logout: 'Logout',
        login: 'Admin Login'
      },
      disclosure: {
        title: 'Disclosure Board',
        desc: 'Check notices and disclosure information from Orcinus',
        notice: 'Notices',
        general: 'General Disclosure',
        demoTab: 'Demo Requests',
        loading: 'Loading...',
        noDemo: 'No demo requests yet.',
  ...
  const featureIcons = ["activity", "zap", "shield", "target", "layers"];
  const features = txt.features.items.map((item, i) => ({
    icon: featureIcons[i],
    title: item.title,
    desc: item.desc,
  }));

  const clientIcons = ["building", "briefcase", "pieChart", "activity"];
  const clients = txt.clients.items.map((item, i) => ({
    icon: clientIcons[i],
    title: item.title,
  }));
  ...
  useEffect(() => {
    ...
    fetch(url)
      ...
      .catch(() => {
        setDisclosureError(txt.disclosure.fetchError);
        setDisclosureLoading(false);
      });
  }, [currentPage]);
    ...
    return (
      <div className="fixed inset-0 z-[200] flex items-center justify-center px-4"
        ...
        <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 z-10 text-center">
          ...
          <h3 className="text-lg font-black text-slate-900 mb-1">{txt.admin.loginTitle}</h3>
          <p className="text-slate-400 text-xs mb-6">{txt.admin.loginDesc}</p>
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg p-8 z-10">
            ...
            {demoStatus === 'success' ? (
              <div className="text-center py-8">
                ...
                <h3 className="text-xl font-black text-slate-900 mb-2">{txt.demo.successTitle}</h3>
                <p className="text-slate-500 text-sm mb-6">{txt.demo.successDesc}</p>
                <button onClick={() => { setShowDemoModal(false); setDemoStatus('idle'); }}
              ...
              <>
                <div className="mb-6">
                  ...
                  <h3 className="text-2xl font-black text-slate-900 mt-1">{txt.demo.title}</h3>
                  <p className="text-slate-400 text-sm mt-1">{txt.demo.desc}</p>
                </div>
                <div className="space-y-3">
                  <div>
                    <label className="text-xs font-semibold text-slate-600 mb-1 block">{txt.demo.nameLabel} <span className="text-cyan-500">*</span></label>
                    <input type="text" placeholder={txt.demo.namePlaceholder} value={demoForm.name}
                  ...
                  <div>
                    <label className="text-xs font-semibold text-slate-600 mb-1 block">{txt.demo.companyLabel} <span className="text-cyan-500">*</span></label>
                    <input type="text" placeholder={txt.demo.companyPlaceholder} value={demoForm.company}
                      onChange={e => setDemoForm(f => ({ ...f, company: e.target.value }))}
                      className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-800 placeholder-slate-300 focus:outline-none focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/20 transition-all" />
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-slate-600 mb-1 block">{txt.demo.phoneLabel}</label>
                    <input type="text" placeholder="010-0000-0000" value={demoForm.phone}
                      onChange={e => setDemoForm(f => ({ ...f, phone: e.target.value }))}
                      className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-800 placeholder-slate-300 focus:outline-none focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/20 transition-all" />
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-slate-600 mb-1 block">{txt.demo.emailLabel} <span className="text-cyan-500">*</span></label>
                    <input type="text" placeholder="example@company.com" value={demoForm.email}
                      onChange={e => setDemoForm(f => ({ ...f, email: e.target.value }))}
                      className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-800 placeholder-slate-300 focus:outline-none focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/20 transition-all" />
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-slate-600 mb-1 block">{txt.demo.messageLabel}</label>
                    <textarea placeholder={txt.demo.messagePlaceholder}
                ...
                {demoStatus === 'error' && (
                  <p className="text-red-500 text-xs mt-2">{txt.demo.errorMsg}</p>
                )}
                <button onClick={handleDemoSubmit}
                  ...
                  {demoStatus === 'loading'
                    ? <><div className="w-4 h-4 border-2 border-black/30 border-t-black rounded-full animate-spin" /> {txt.demo.sending}</>
                    : txt.demo.submit}
                </button>
                <p className="text-center text-xs text-slate-300 mt-3">{txt.demo.required}</p>
              </>
            )}
          </div>
        </div>
